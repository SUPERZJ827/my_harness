...
class OpenAILLM(BaseLLM):
    ...
    async def _achat_completion_stream(self, messages: list[dict], timeout=USE_CONFIG_TIMEOUT) -> str:
        ...
        logger.info(
            f"LLM stream request start: model={self.model}, base_url={self.config.base_url}, timeout={request_timeout}"
        )
        try:
            response: AsyncStream[ChatCompletionChunk] = await self.aclient.chat.completions.create(
                **self._cons_kwargs(messages, timeout=request_timeout), stream=True
            )
        except Exception as e:
            logger.exception(
                f"LLM stream request failed: model={self.model}, base_url={self.config.base_url}, timeout={request_timeout}, error={e}"
            )
            raise
        usage = None
        collected_messages = []
    ...
    async def _achat_completion(self, messages: list[dict], timeout=USE_CONFIG_TIMEOUT) -> ChatCompletion:
        request_timeout = self.get_timeout(timeout)
        kwargs = self._cons_kwargs(messages, timeout=request_timeout)
        logger.info(
            f"LLM request start: model={self.model}, base_url={self.config.base_url}, timeout={request_timeout}"
        )
        try:
            rsp: ChatCompletion = await self.aclient.chat.completions.create(**kwargs)
        except Exception as e:
            logger.exception(
                f"LLM request failed: model={self.model}, base_url={self.config.base_url}, timeout={request_timeout}, error={e}"
            )
            raise
        self._update_costs(rsp.usage)
        logger.info(f"LLM request finished: model={self.model}")
        return rsp
