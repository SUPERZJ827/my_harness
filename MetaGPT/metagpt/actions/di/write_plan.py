...
def parse_plan_json_lenient(rsp: str) -> str:
    fenced = re.search(r"```json\s*(.*?)```", rsp, flags=re.DOTALL | re.IGNORECASE)
    if fenced:
        return fenced.group(1).strip()
    # Avoid treating Python code blocks as plans. Only accept a JSON array.
    start = rsp.find("[")
    end = rsp.rfind("]")
    if 0 <= start < end:
        candidate = rsp[start : end + 1].strip()
...
class WritePlan(Action):
    ...
    async def run(self, context: list[Message], max_tasks: int = 5) -> str:
        ...
        return parse_plan_json_lenient(rsp)
