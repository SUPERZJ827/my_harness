...
class Planner(BaseModel):
    ...
    auto_run: bool = False
    plan_writter: WritePlan = None

    def __init__(self, goal: str = "", plan: Plan = None, **kwargs):
        plan = plan or Plan(goal=goal)
        super().__init__(plan=plan, **kwargs)

    def set_plan_writter(self, config: Config):
        self.plan_writter = WritePlan(config=config)
        # self.plan_writter.llm.config = config.llm
        # print("Plan Writter Config:\n", self.plan_writter.llm.config)
    ...
    async def update_plan(self, goal: str = "", max_tasks: int = 3, max_retries: int = 3):
        ...
        while not plan_confirmed:
            ...
            rsp = await self.plan_writter.run(context, max_tasks=max_tasks)
            ...
            if not is_plan_valid:
                logger.warning(f"Plan generation failed after retries, using fallback plan. Last error: {error}")
                if self.plan.current_task:
                    rsp = json.dumps([self.plan.current_task.dict()], ensure_ascii=False)
                else:
                    rsp = json.dumps(
                        [
                            {
                                "task_id": "1",
                                "dependent_task_ids": [],
                                "instruction": "Solve the user requirement and create the required output artifacts",
